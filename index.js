...
async function decryptFile(buffer, key) {
  try {
    console.log(`Decrypting file. Buffer length: ${buffer.length}`);
    const salt = buffer.slice(0, 64);
    const iv = buffer.slice(64, 80);
    const authTag = buffer.slice(80, 96);
    const encrypted = buffer.slice(96);
    console.log(`Salt length: ${salt.length}, IV length: ${iv.length}, AuthTag length: ${authTag.length}, Encrypted data length: ${encrypted.length}`);
    
    // Convert the key from hex string to buffer
